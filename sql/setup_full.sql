-- sql/setup_full.sql
-- 1) Criar banco e usuário (ajuste a senha se quiser)
CREATE DATABASE IF NOT EXISTS acesso CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
CREATE USER IF NOT EXISTS 'acesso_user'@'localhost' IDENTIFIED BY 'acesso123';
GRANT ALL PRIVILEGES ON acesso.* TO 'acesso_user'@'localhost';
FLUSH PRIVILEGES;
USE acesso;
...
-- 3) Ajustes de ambientes (opcional; se já existirem as tabelas acima, esses comandos adaptam para suporte ao "dispositivo principal"):
ALTER TABLE environments ADD COLUMN IF NOT EXISTS primary_device_id BIGINT UNSIGNED NULL AFTER env_type;
ALTER TABLE environments ADD CONSTRAINT IF NOT EXISTS fk_env_primary_device FOREIGN KEY (primary_device_id) REFERENCES devices(id) ON DELETE SET NULL;

CREATE TABLE IF NOT EXISTS environment_devices_aux (
  environment_id BIGINT UNSIGNED NOT NULL,
  device_id BIGINT UNSIGNED NOT NULL,
